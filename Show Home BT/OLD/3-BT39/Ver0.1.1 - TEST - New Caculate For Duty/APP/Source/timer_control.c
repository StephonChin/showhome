...
__interrupt void Tim4_Update_Interrupt(void)
{
  ...
  TIM4_SR &= (~0x01);
	...
	if ( (DimmerDutyCnt == (150 - DimmerDuty)) && (DimmerDuty > 0) )
	{
		PORT_SWITCH = 1;
	}
...
void Timer_Control(void)
{
		...
		/* 实际占空比计算 f(x) = 150 - 0.015 *(DimmerDuty - 100)^2 */
		TempVal = (_Uint)(100 - DimmerDuty);
		TempVal = TempVal * TempVal;
		TempVal = TempVal * 3 / 200;
		DimmerDutyVal	= 150 - (_Uchar)TempVal;
