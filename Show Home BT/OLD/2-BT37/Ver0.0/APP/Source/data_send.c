...
void Respond_App(void)
{
    ...
    if (StatusReplyFlag) 
    {
		if (StatusBothReplyFlag == FALSE)
		{
			...
			{
				RepeatDelayTime++;
				if (RepeatDelayTime > 4) // 5 * 10ms = 50ms
				{
					RepeatDelayTime = 0;
					ReplyRepeatFlag =TRUE;
		...
		else
		{
			if (ReplyRepeatFlag)  //send repeat
			{
				...
				Respond_Byte(StatusReplyCommand);  // 0x02 | (flag | 0x02)
				Respond_Byte(ReplyGroup); 
				Respond_Byte(SwitchStatus[ReplyGroup]);
				Respond_Byte(0x00);
				Respond_Byte(PACKAGE_TAIL);   // 0xFE
			} //send repeat
