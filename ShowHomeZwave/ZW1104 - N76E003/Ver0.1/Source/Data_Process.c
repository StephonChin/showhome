...
/* @File Variables -----------------------------------------*/
static _Flag	TamperCloseFlag;
static _Flag	ConnectFlag;
static _Flag	SensorFlag;
static _Flag	PowerOnFlag;
enum
{
	SENSOR_IDLE,
	SENSOR_CLOSE,
...
void Data_Process(void)
{
	...
	if (NetStatus != NET_IDLE)
	{
		...
		if (ShakeTime > 2)
		{
			...
			if (NetStatus == NET_CONNECT && PORT_DEV_ADD == RESET && NetPreStatus == NET_DISCONNECT)
			{
				ShakeFlag					= TRUE;
				ConnectFlag 			= TRUE;
				LedData.ModeBuf		= LED_G_FAST;
				if (PowerOnFlag != TRUE)			NetChgFlag = TRUE;
				NetPreStatus = NetStatus;
			}
			else if (NetStatus == NET_DISCONNECT && PORT_DEV_ADD == SET && NetPreStatus == NET_CONNECT)
			{
				ShakeFlag					= TRUE;
				ConnectFlag				= FALSE;
